...
test.describe('Webhook Flow', () => {
  ...
  test('TC-WF-003: Add JS Code step, use Ask AI with a prompt, test and save', async ({ page }) => {
    ...
    // Click empty canvas area to dismiss the variable popover (popover close button removed from UI)
    ...
    // MUI Select renders connection items in a portal — use getByRole('option') scoped inside the listbox
    ...
    // data-testid='plugin-field-channel_id' (pluginHocV2.tsx line 592)

    // await page.getByTestId('plugin-field-channel_id').click();
